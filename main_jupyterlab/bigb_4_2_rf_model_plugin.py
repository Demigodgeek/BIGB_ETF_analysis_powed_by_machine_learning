...
# Apply the tuned X, y, and examing period to the second machine learning model - Random Forest, then backtest and evaluate the model. 
...
def bigb_rf_model(X_train_scaled, X_test_scaled, y_train, y_test, signals_df):
    ...
    plot_actual_vs_strategy_returns_rf = (1 + bigb_predictions_df_rf[["actual_returns", "strategy_returns"]]).cumprod().plot(
        title="BIG BANK - Actual Returns vs Strategy Returns - Random Forest",
        xlabel="Date",
        ylabel="Cumulative Returns"
    )
    plt.savefig("images/BIG BANK - Actual Returns vs Strategy Returns - Random Forest.png")
