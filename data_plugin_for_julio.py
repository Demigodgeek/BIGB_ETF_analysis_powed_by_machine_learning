...
def get_bigb_data():
    ...
    # careate a dataframe contains only 'close' and 'actual_return' which can be used for trading_algorithm analysis
    bigb_trading_df = bigb_df[['close', 'actual_returns']].copy().dropna()
    ...
    bigb_3years_df = bigb_df.loc[start_date_3years_ago:end_date]
...
def get_sp500_data():
    ...
    sp500_trading_df = pd.DataFrame({"close": sp500_df["close"],"actual_return": sp500_daily_returns})
    # drop the first row since it contains a NaN value
    sp500_trading_df = sp500_trading_df.dropna()
     
    return sp500_trading_df