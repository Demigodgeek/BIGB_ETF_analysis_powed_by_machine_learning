...
def bigb_svc_model(X_train_scaled, X_test_scaled, y_train, y_test, signals_df):
    ...
    # the actual returns versus the strategy returns
    
    actual_vs_strategy_returns_svc = (1 + bigb_predictions_df_svc[["actual_returns", "strategy_returns"]]).cumprod()
    
    # Create a line plot of the cumulative returns
    plot_actual_vs_strategy_returns_svc, ax = plt.subplots(figsize=(10, 6))
    ax.plot(actual_vs_strategy_returns_svc)
    ax.legend(actual_vs_strategy_returns_svc.columns)
    ax.set_xlabel('Date')
