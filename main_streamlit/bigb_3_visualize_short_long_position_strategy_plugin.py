...
def visualize_short_position_strategy(bigb_trading_df):
    ...
    # Overlay the plots
    sp_entry_exit = sp_security_close * sp_moving_avgs * sp_entry * sp_exit
    sp_entry_exit.opts(
        title='BIGB - Short-Position Dual Moving Average Trading Algorithm'
    )
    sp_entry_exit_plot= hv.render(sp_entry_exit, backend='bokeh')
...
def visualize_long_position_strategy(bigb_trading_df):
    ...
    # Overlay the plots
    lp_entry_exit = lp_security_close * lp_moving_avgs * lp_entry * lp_exit
    lp_entry_exit.opts(
        title='BIGB - Long-Position Dual Moving Average Trading Algorithm'
    )
    lp_entry_exit_plot= hv.render(lp_entry_exit, backend='bokeh')
