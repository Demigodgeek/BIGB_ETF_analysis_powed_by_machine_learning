...
def bigb_knn_bestk_model(X_train_scaled, X_test_scaled, y_train, y_test, signals_df):
    ...
    bigb_actual_vs_strategy_returns_knn = (1 + bigb_predictions_df_knn[["actual_returns", "strategy_returns"]]).cumprod()
    
    # Create a line plot of the cumulative returns
    plot_bigb_actual_vs_strategy_returns_knn, ax = plt.subplots(figsize=(10, 6))
    ax.plot(bigb_actual_vs_strategy_returns_knn)
    ax.legend(bigb_actual_vs_strategy_returns_knn.columns)
    ax.set_xlabel('Date')
